  ...
  try {
    ...
    const usersWithStatus = await Promise.all(
      data.map(async (user: any) => {
        const banRecord = Array.isArray(user.banned_users)
          ? user.banned_users[0]
          : user.banned_users;
...
export const banUserController = async (req: Request, res: Response) => {
  try {
    ...
    switch (duration_unit) {
      case "minutes":
        expiresAt.setMinutes(expiresAt.getMinutes() + parseInt(duration_value));
        break;
      case "hours":
        expiresAt.setHours(expiresAt.getHours() + parseInt(duration_value));
        break;
      case "days":
        expiresAt.setDate(expiresAt.getDate() + parseInt(duration_value));
        break;
      case "weeks":
        expiresAt.setDate(expiresAt.getDate() + parseInt(duration_value) * 7);
        break;
      case "months":
        expiresAt.setMonth(expiresAt.getMonth() + parseInt(duration_value));
        break;
      case "years":
        expiresAt.setFullYear(
          expiresAt.getFullYear() + parseInt(duration_value),
        );
        break;
      default:
        return res.status(400).json({ error: "Format durasi tidak valid." });
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Pengguna berhasil diban.", data });
  } catch (err: any) {
    return res
      .status(500)
      .json({ error: err.message || "Internal Server Error" });
...
export const unbanUserController = async (req: Request, res: Response) => {
  try {
    ...
    return res
      ...
      .json({
        success: true,
        message: "Status banned berhasil dicabut. Pengguna sekarang ACTIVE.",
      });
  } catch (err: any) {
    return res
      .status(500)
...
export const submitAppealController = async (req: Request, res: Response) => {
  try {
    ...
    if (!banRecord) {
      ...
      return res
        .status(400)
        .json({ error: "Pengguna tidak dalam status penangguhan (banned)." });
    }
    ...
    if (banRecord.appeal_reason && banRecord.appeal_text) {
      return res
        ...
        .json({
          error:
            "Anda sudah mengajukan banding sebelumnya. Harap tunggu tinjauan admin.",
        });
    }
    ...
    return res
      ...
      .json({
        success: true,
        message: "Banding berhasil dikirim. Menunggu tinjauan admin.",
      });
  } catch (err: any) {
    return res
      .status(500)
...
export const getAppealsController = async (req: Request, res: Response) => {
  try {
    ...
    const validAppeals = data.filter(
      (item: any) => item.appeal_reason && item.appeal_text,
    );
