...
export const createDonation = async (req: Request, res: Response) => {
  try {
    ...
    if (userError || !userData) {
      return res.status(404).json({ success: false, message: 'Akun pengguna tidak ditemukan di database' });
    }
...
export const donationWebhook = async (req: Request, res: Response) => {
  try {
    const { order_id, transaction_status, custom_field1, gross_amount } = req.body;
    ...
    if (finalStatus !== 'pending') {
      await supabase.from('donation_users').update({ status: finalStatus }).eq('order_id', order_id);
    }
...
export const getDonationHistory = async (req: Request, res: Response) => {
  try {
    ...
    let query = supabase.from('donation_users').select('*').order('created_at', { ascending: false });
    ...
    if (data && data.length > 0) {
      ...
      for (let donation of pendingDonations) {
        try {
          const midtransStatus = await snap.transaction.status(donation.order_id);
          ...
          if (finalStatus !== 'pending') {
            await supabase.from('donation_users').update({ status: finalStatus }).eq('order_id', donation.order_id);
    ...
    res.status(500).json({ success: false, message: 'Gagal mengambil riwayat donasi' });
  }
};
...
export const updateDonationStatus = async (req: Request, res: Response) => {
  try {
    ...
    const { data, error } = await supabase.from('donation_users').update({ status }).eq('id', id).select();
    if (error) throw error;
    res.status(200).json({ success: true, message: 'Status berhasil diubah', data });
  } catch (error) {
...
export const deleteDonation = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { error } = await supabase.from('donation_users').delete().eq('id', id);
    if (error) throw error;
    res.status(200).json({ success: true, message: 'Riwayat donasi berhasil dihapus' });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Gagal menghapus riwayat donasi' });
  }
};