  ...
  try {
    ...
    if (error || !pending) {
      return res
        .status(400)
        .json({
          error: "Data registrasi tidak ditemukan atau sudah kedaluwarsa.",
        });
    }
    ...
    if (new Date() > new Date(pending.expired_at)) {
      return res
        .status(400)
        .json({ error: "Kode OTP sudah kedaluwarsa (lebih dari 30 menit)." });
    }
...
export const resendOtpController = async (req: Request, res: Response) => {
  try {
    ...
    if (!identifier) {
      return res
        .status(400)
        .json({ error: "Identitas (Email/Username/Telepon) wajib diisi." });
    }
    ...
    if (pending && pending.username !== "LOGIN_SESSION") {
      await supabase
        .from("pending_users")
        .update({ otp_code: newOtp, expired_at: newExpired })
        .eq("id", pending.id);

      const emailHtml = getRegisterMailTemplate(pending.full_name, newOtp);
      await sendOTPEmail(
        pending.email,
        "Kirim Ulang Verifikasi Akun - XyNest Project",
        emailHtml,
      );
      ...
      return res
        ...
        .json({
          success: true,
          message: "Kode OTP baru berhasil dikirim ulang ke email Anda.",
        });
    }
    ...
    if (user) {
      ...
      await sendOTPEmail(
        user.email,
        "Kirim Ulang Otentikasi Sesi Masuk - XyNest Project",
        emailHtml,
      );
      return res
        .status(200)
        .json({
          success: true,
          message: "Kode OTP baru untuk login berhasil dikirim ulang.",
        });
    }
